...
module.exports = function(app, saveSecurityConfig) {
  ...
  app.get('/security/config', (req, res, next) => {
    if (
      ...
      res.json(app.securityStrategy.getConfig())
    } else {
  ...
  app.put('/security/config', (req, res, next) => {
    if (
      ...
      var config = app.securityStrategy.setConfig(req.body, err => {
        if (err) {
          console.log(err)
          res.status(500)
  ...
  app.get('/security/users', (req, res, next) => {
    if (
      ...
      res.json(app.securityStrategy.getUsers())
    } else {
      res.status(401).json('Security config not allowed')
    }
  })
  ...
  app.put('/security/users/:id', (req, res, next) => {
    if (
      ...
      app.securityStrategy.updateUser(req.params.id, req.body, err => {
        if (err) {
          console.log(err)
          res.status(500)
  ...
  app.post('/security/users/:id', (req, res, next) => {
    if (
      ...
      app.securityStrategy.addUser(user, err => {
        if (err) {
          console.log(err)
          res.status(500)
          res.send(_.isString(err) ? err : 'Unable to add user')
        } else {
          res.send('User added')
        }
      })
  ...
  app.put('/security/user/:username/password', (req, res, next) => {
    if (
      ...
      app.securityStrategy.setPassword(req.params.username, req.body, err => {
        if (err) {
          console.log(err)
          res.status(500)
  ...
  app.delete('/security/users/:username', (req, res, next) => {
    if (
      ...
      app.securityStrategy.deleteUser(req.params.username, (err, config) => {
        if (err) {
          console.log(err)
          res.status(500)
  ...
  if (
    ...
    app.post('/enableSecurity', (req, res, next) => {
      if (app.securityStrategy == null) {
        ...
        config.writeSettingsFile(app, app.config.settings, err => {
          if (err) {
            ...
            addUser(req, res, securityStrategy)
          }
        })
      } else {
      ...
      function addUser(req, res, securityStrategy) {
        securityStrategy.addUser(req.body, (err, config) => {
          if (err) {
            console.log(err)
            res.status(500)
            res.send('Unable to add user')
